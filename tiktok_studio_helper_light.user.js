// ==UserScript==
// @name         TikTok Studio — Combined Helper v7.6
// @namespace    https://tampermonkey.net/
// @version      7.6
// @description  Панель TikTok Studio. Основа — оригинальный scheduler v4 (клики по пикерам). Порядок: дата→время.
// @author       FANTOM + merged
...
function log(...args) {
  ...
  const box = document.getElementById('tt-log');
  if (!box) return;
  const d = document.createElement('div');
  const t = new Date().toLocaleTimeString();
  const msg = args.map(x=>typeof x==='string'?x:JSON.stringify(x)).join(' ');
  d.innerHTML = '<span style="color:#1a4040">['+t+']</span> <span style="color:#4a8080">'+escapeHtml(msg)+'</span>';
  box.appendChild(d);
...
function setButtonsBusy(busy) {
  ['tt-parse','tt-fill','tt-schedule','tt-upload','tt-all'].forEach(id=>{
...
/* ── PANEL ── */
#${PANEL_ID}{
  position:fixed;top:64px;right:14px;width:360px;max-height:93vh;
...
<div class="body">
  ...
  <div class="sec" id="sec-actions">
    <div class="sec-hdr" data-sec="actions">
      <div class="sec-hdr-left">
        ...
        <span class="sec-title">Действия по отдельности</span>
      </div>
  ...
  <div class="sec" id="sec-timings">
    <div class="sec-hdr" data-sec="timings">
      <div class="sec-hdr-left">
        <span class="sec-icon">⚙</span>
        <span class="sec-title">Тайминги</span>
      </div>
  ...
  <div class="sec" id="sec-preview">
    <div class="sec-hdr" data-sec="preview">
      <div class="sec-hdr-left">
        <span class="sec-icon">🔎</span>
        <span class="sec-title">Разбор строки</span>
      </div>
  ...
  log('v7.2 started (separate caption+tags fill)');
  const saved=storageGet(STORAGE_KEY,'');
  if (saved) document.getElementById('tt-input').value=saved;
  ...
  document.getElementById('tt-tm-save').onclick=function(){
    ...
    setStatus('ok','✅ Тайминги сохранены.'); log('timings saved');
  };
